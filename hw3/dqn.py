...
class QLearner(object):
    ...
    def __init__(
        ...
        self.all_q_values = q_func(obs_t_float, self.num_actions, scope="q_func", reuse=False)
        ...
        q_values = tf.gather_nd(self.all_q_values, action_index)
        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
    ...
    def step_env(self):
        ...
        if not self.model_initialized or np.random.rand() < esp:
            ...
            all_q_values = self.session.run(self.all_q_values, feed_dict={self.obs_t_ph: [obs]})[0]
            action = np.argmax(all_q_values)
    ...
    def update_model(self):
        ...
        if (self.t > self.learning_starts and self.t % self.learning_freq == 0 and self.replay_buffer.can_sample(
                self.batch_size)):
