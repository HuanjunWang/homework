...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None,
    ...
    with tf.variable_scope(scope):
        ...
        for _ in range(n_layers):
            x = tf.layers.dense(inputs=x, units=size, activation=activation, trainable=trainable)
...
class Agent(object):
    ...
    #                           ----------PROBLEM 2----------
    # ========================================================================================#
    def policy_forward_pass(self, sy_ob_no, scope="", trainable=True):
        ...
        with tf.variable_scope(scope):
            if self.discrete:
                sy_logits_na = build_mlp(sy_ob_no, output_size=self.ac_dim, scope="PolicyNetwork",
                                         n_layers=self.n_layers,
                                         size=self.size,
                                         activation=tf.nn.relu, output_activation=None, trainable=trainable)
                return sy_logits_na
            else:
                sy_mean = build_mlp(sy_ob_no, output_size=self.ac_dim, scope="PolicyMean", n_layers=self.n_layers,
                                    size=self.size,
                                    activation=tf.nn.relu, output_activation=None, trainable=trainable)
                sy_logstd = tf.get_variable('PolicyLogstd', shape=[self.ac_dim], trainable=trainable)
    ...
    def build_computation_graph(self, logdir):
        ...
        self.pp = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'new')

        self.old_policy_parameters = self.policy_forward_pass(self.sy_ob_no, scope="old", trainable=False)
        self.old_pp = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'old')

        self.new2old = [tf.assign(oldv, newv) for (oldv, newv) in zip(self.old_pp, self.pp)]
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            ac = self.sess.run(self.sy_sampled_ac, feed_dict={self.sy_ob_no: ob[np.newaxis, :]})  # TODO Debug
            ac = ac[0]
            acs.append(ac)
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            for path in re_n:
                reward = list()
                reward.append(path[-1])
                for r in path[-2:: -1]:
                    reward.append(r + reward[-1] * self.gamma)
                reward.reverse()
                q_n.extend(reward)
        else:
            q_n = []
